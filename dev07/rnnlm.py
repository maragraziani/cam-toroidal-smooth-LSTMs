  ...
  def fit(self, valid_data_list, trn_data_list, learning_rate=1e-2, lr_decay=0.8, batch_size=50, dropout=1.0, optimizer=tf.train.AdamOptimizer, n_epochs=30, stimulated=0):
    ...
    with self._graph.as_default():
      ...
      if stimulated:
          ...
          _regularisation(transformed_activations[0], target)
      ...
      for epoch in xrange(1, n_epochs+1):
        ...
        if True:
            ...
            save_evolution(self.sess, epoch,reshaped_activations[0][0][0])
...
'''##### ##### ###  VISUALISATION  ### ##### #####'''
def save_evolution(sess, epoch, word_activation):
    activation_grid=sess.run(word_activation)
    plt.imshow(activation_grid, interpolation='None', cmap=cm.jet)
    plt.savefig('raw_activations/'+str(epoch)+'.png')
    return
