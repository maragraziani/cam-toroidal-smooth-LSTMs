...
class RNNLM(BaseModel):
  ...
  def fit(self, valid_data_list, trn_data_list, learning_rate=1e-2, lr_decay=0.8, batch_size=50, dropout=1.0, optimizer=tf.train.AdamOptimizer, n_epochs=30, stimulated=0, regweight=1e-4, imshape=14):
    ''''
    ...
    with self._graph.as_default():
      ...
      if stimulated:
          ...
          transformed_activations = _activation_transformation(reshaped_activations, imshape=imshape) # np.shape(reshaped_activations[0]) = (batch_size, bppt)t
          ...
          regularizer = _regularisation(transformed_activations, target, seqlens=self.data_queue_list[2], regweight=regweight)
...
def _node_organisation(activations, batch_size=64, bptt=20, imshape=14, save_folder='raw_activations'):
  '''
...
def hypersphere(reshaped_activations, imshape=14):
    ...
    #new=tf.Variable(tf.zeros((dim[0]+2, dim[1]+2)))
    # assignPatch = new[1:dim[0]+1, 1:dim[1]+1].assign(patch)
    #
    # #1. head line = last row
    # headLine = new[0, 1:dim[1]+1].assign(patch[dim[0]-1, :dim[1]])
    # #2. end line = top row
    # endLine = new[dim[0]+1, 1:dim[1]+1].assign(patch[0, :])
    # #3. column left = last right column
    # colLeft= new[1:dim[0]+1, 0].assign(patch[:dim[0], dim[1]-1 ])
    # #4. column right = first left column
    # colRight= new[1:dim[0]+1, dim[1]+1].assign(patch[:dim[0], 0])
    #
    # #5. edges
    # # ...bottom-right -> left-top
    # eBottomRight = new[0, 0].assign(patch[dim[0]-1, dim[1]-1])
    # # ...bottom-left -> right-top
    # eBottomLeft=new[0, dim[1]+1].assign(patch[dim[0]-1, 0])
    # # ...top-left -> right-bottom
    # eTopLeft=new[dim[0]+1, dim[1]+1].assign(patch[0, 0])
    # # ...top-right -> left-bottom
    # eTopRight= new[dim[1]+1, 0 ].assign(patch[0, dim[1]-1])
    #
    # karnaughOperations=[assignPatch, headLine, endLine, colLeft, colRight, eBottomRight, eBottomLeft, eTopLeft, eTopRight]
    #return karnaughOperations
...
def _activation_transformation(reshaped_activations, transformation = 'high_pass', batch_size=64, bptt=20, imshape=14, save_folder='hp_activations'):
    ...
    #         x = tf.reshape(reshaped_activations[sentence][word], (1, imshape, imshape, 1))
    #
...
def _activation_target(imshape=14):
    '''
...
def _regularisation(transformed_activations, target, seqlens,regweight=1e-4,  type=1, batch_size=64, bppt=20):
    '''
