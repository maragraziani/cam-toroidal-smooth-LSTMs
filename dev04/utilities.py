...
def process_data_lm(data, path, input_index, output_index, bptt, spId=False):
  ...
  if bptt==None:
    ...
    for i, length in zip(xrange(len(in_data)), slens):
      ...
      #THIS IS RIGHT. You skip the first word in output and look at the next words
      ...
      for j in xrange(lines):
        ...
        target_processed_data[row+j][0:len(output[j*bptt:(j+1)*bptt])] = output[j*bptt:(j+1)*bptt]
      row+=lines
