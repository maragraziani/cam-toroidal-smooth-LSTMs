...
class RNNLM(BaseModel):
  ...
  def __init__(self, network_architecture=None, name=None, dir=None, load_path=None, debug_mode=0, seed=100):
    ...
    with self._graph.as_default():
      ...
      with tf.variable_scope('model') as scope:
        ...
        self.predictions, self.logits, reshaped_activation = self._construct_network(input=self.x,
  ...
  def _construct_network(self, input, seqlens, batch_size, WD=1e-6, keep_prob=1.0):
    ...
    return predictions, logits, activations
  ...
  def fit(self, valid_data_list, trn_data_list, learning_rate=1e-2, lr_decay=0.8, batch_size=50, dropout=1.0, optimizer=tf.train.AdamOptimizer, n_epochs=30, stimulated=0):
    ...
    with self._graph.as_default():
      ...
      with tf.variable_scope(self._model_scope, reuse=True) as scope:
        self.trn_predictions, self.trn_logits, activations = self._construct_network(input=self.data_queue_list[1],
      ...
      if stimulated:
          ...
          transformed_activations = _activation_transformation(reshaped_activations[0]) # np.shape(reshaped_activations[0]) = (batch_size, bppt)t
          #import pdb; pdb.set_trace()
          ...
          _regularisation(transformed_activations[0], target, seqlens=self.data_queue_list[2])
      ...
      for epoch in xrange(1, n_epochs+1):
        ...
        if stimulated:
            ...
            save_evolution(self.sess, epoch,tf.reshape(transformed_activations[0][0][0], [14,14]), os.path.join(self._dir, 'filtered_activations'))
...
def _node_organisation(activations, batch_size=10, bptt=20, shape=14, save_folder='raw_activations'):
  ...
  for sentence in range(batch_size):
      ...
      for word in range(bptt):
          ...
          reshaped_act= tf.reshape(act, [shape, shape])
...
def _activation_target(imsize=14):
    '''
    ...
    '''
    target = tf.zeros([imsize,imsize])

    return target

def _regularisation(transformed_activations, target, seqlens, type=1, batch_size=10, bppt=20):
    '''
    ...
    for sentence in range(batch_size):
        for word in range(bppt):
            ...
            regularizer=tf.add(regularizer, tf.norm(distance))
