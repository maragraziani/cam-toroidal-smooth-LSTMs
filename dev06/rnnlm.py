...
def _node_organisation(activations, batch_size=10, bptt=20, shape=14):
  '''
...
def _activation_transformation(reshaped_activations, transformation = 'high_pass', batch_size=10, bptt=20, imshape=14):
    ...
    with tf.Session() as sess:
        ...
        lowpass = tf.nn.conv2d(reshaped_activations[0][0], w, strides=[1, 1, 1, 1], padding='SAME')
        highpass = x-lowpass
    ...
    # for sentence in range(batch_size):
    #     for word in range(bptt):
    #         '[debug] currently in {0} sentence, {1} word'.format(sentence, word)
    #
    #
    #
    #         image = reshaped_activations[sentence][word]
    #         import pdb; pdb.set_trace()
