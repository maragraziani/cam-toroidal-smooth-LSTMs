  ...
  def __init__(self, network_architecture=None, name=None, dir=None, load_path=None, debug_mode=0, seed=100):
    ...
    with self._graph.as_default():
      ...
      with tf.variable_scope('model') as scope:
        ...
        self._activations=reshaped_activation
   ...
   n_epochs=30, stimulated=0, regtype='mse', regweight=1e-4, imshape=14, hypersphere=0, transformation='high_pass', cell_type='lstm'):
    ...
    with self._graph.as_default():
      ...
      with tf.variable_scope(self._model_scope, reuse=True) as scope:
        self.trn_predictions, self.trn_logits, self._activations, inl2, softmaxl2 = self._construct_network(input=self.data_queue_list[1],
      ...
      if stimulated:
          ...
          self._normed_activation, self._normed_target, regularizer = _regularisation(transformed_activations, target, seqlens=self.data_queue_list[2], regtype=regtype, regweight=regweight, batch_size=batch_size)
      ...
      for epoch in xrange(1, n_epochs+1):
        ...
        if stimulated:
            save_evolution(self.sess, epoch,reshaped_activations, valid_data_list,os.path.join(self._dir, 'raw_activations'))
            save_evolution(self.sess, epoch,transformed_activations, valid_data_list, os.path.join(self._dir, 'filtered_activations'))
  ...
  def predict(self, X):
    ...
    with self._graph.as_default():
      ...
      for i in xrange(len(X[0])/batch_size):
        test_loss_batch, batch_logits, batch_predictions, activations = self.sess.run([evl_cost, self.logits, self.predictions, self._activations],
                                                                            feed_dict={self.y : X[0][i*batch_size:(i+1)*batch_size],
                                                                            self.x : X[1][i*batch_size:(i+1)*batch_size],
                                                                            self.seqlens : X[2][i*batch_size:(i+1)*batch_size],
      ...
      np.save(self._dir+'/debug_activations', activations)
      data=X[0]
      np.save(self._dir+'/input', data)

      np.save(self._dir+'/predictions', predictions)
      print 'Test PPL', np.exp(test_loss)

  def rescore(self, X, batch_size=1):
    ...
    with self._graph.as_default():
      ...
      batch_range=np.arange(batch_size)[:,np.newaxis]
      time_range=np.arange(X[0].shape[1])[np.newaxis,:]
      for i in xrange(len(X[0])/batch_size):
        batch_predictions = self.sess.run(self.predictions, feed_dict={self.y : X[0][i*batch_size:(i+1)*batch_size], self.x : X[1][i*batch_size:(i+1)*batch_size], self.seqlens : X[2][i*batch_size:(i+1)*batch_size], self.batch_size : batch_size})

        if len(predictions)==0:
                predictions=batch_predictions[batch_range,time_range,X[0][batch_range,time_range]]
        else:
                predictions=np.concatenate([predictions,batch_predictions[batch_range,time_range,X[0][batch_range,time_range]]], axis=0)
        print 100.0*float((i+1)*batch_size)/len(X[0]), "% done"
...
def _activation_target(words,target_type, imshape=14, batch_size=64, bptt=20):
    ...
    if target_type=='cos':
        ...
        targets=np.load('manualTargs.npy')
...
def _regularisation(transformed_activations, target, seqlens, regtype='mse' ,regweight=1e-4,  type=1, batch_size=64, bppt=20):
    ...
    return normed_activation, normed_target, regularizer
'''#####################################################END######################################################################'''

'''##### ##### ###  VISUALISATION  ### ##### #####'''
def save_evolution(sess, epoch, word_activation, input,folder):
    ...
    data = input
    #import pdb; pdb.set_trace()
    #np.save(folder + '/input_'+str(epoch), input[0])
