...
class RNNLM(BaseModel):
  ...
  stimulated=0, regtype='mse', regweight=1e-4, imshape=14, hypersphere=0, transformation='high_pass'):
    ''''
    ...
    with self._graph.as_default():
      ...
      if stimulated:
          reshaped_activations = _node_organisation(activations, batch_size=batch_size, imshape=imshape, hypersphere=hypersphere)
          ...
          transformed_activations = _activation_transformation(reshaped_activations, transformation=transformation,
                                                                imshape=imshape) # np.shape(reshaped_activations[0]) = (batch_size, bppt)t

          #print '[B. Activation Transformation] Activations transformed with a high pass filter.'
          #import pdb; pdb.set_trace()

          target = _activation_target(self.data_queue_list[1],imshape=imshape)

        #
        #   tftargets=tf.Variable(targets)
        #   input=np.arange(0,len(targets), 1)
      ...
      with open(os.path.join(self._dir, 'LOG.txt'), 'a') as f:
        if stimulated:
            ...
            format_str1 = ('\nRegweight= ' +str(regweight)+ '\nRegtype= '+ regtype +'\nImshape= ' + str(imshape)+'\n'+ 'Hypersphere= ' + str(hypersphere)+'\n')
            format_str2 = ('Transformation= ' +transformation+'\n')
            f.write(format_str)
            f.write(format_str1)
            f.write(format_str2)
...
def _activation_target(words, imshape=14, batch_size=10, bptt=20):
    '''
    ...
    targets=np.loadtxt('posTargets.txt')
    tftargets=tf.Variable(targets)
    input=words
    targs = tf.nn.embedding_lookup(tftargets, input, name='targets')
    target = targs
...
def _regularisation(transformed_activations, target, seqlens, regtype='mse' ,regweight=1e-4,  type=1, batch_size=10, bppt=20):
    '''
    ...
    if regtype=='cos':
        ...
        normed_activation = tf.reshape(tf.nn.l2_normalize(transformed_activations, dim=1), shape=[-1, 196])
        normed_target =  tf.reshape(tf.nn.l2_normalize(target, dim=1), shape=[-1, 196])
        #import pdb; pdb.set_trace()

        #cosine_similarity = tf.matmul(normed_activation, normed_activation)
        #regularizer = cosine_similarity

        #foo = tf.reshape(transformed_activations, shape=[-1, 196])
        #foo = tf.reshape(transformed_activations, shape=[200, 196])
